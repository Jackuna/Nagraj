...
SCHEDULER.every '5s' do
  ...
  host_up = ` curl  -s -k -L -u nagiosadmin:123 'https://nagios.example.com/nagios/cgi-bin/statusjson.cgi?query=hostcount&hoststatus=up+down+unreachable+pending' | jq '.data.count.up' `
  ...
  send_event('hosts_up',   { value: host_up, max: host_count })
  ...
  send_event('downtime_host',   { current: host_under_downtime })
